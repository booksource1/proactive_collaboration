...
class CommunicationAgent(Agent):
    ...
    def plan(
        ...
        if "none" in communication_goal[0] or "None" in communication_goal[0]:
            ...
            for g in communication_goal:
                goal_str += f"{str(g)}\n"
            print()
            print(f"{self.robot_name}'s communication purpose:\n{goal_str}")
            print()
    ...
    def communicate(
        ...
        if self.facts:
            for f in self.facts:
                facts_str += f"{str(f)}\n"
            facts_str = "\n".join(str(self.facts))
        communicate_prompt = self.communicate_prompt.format(
            robot_name=self.robot_name,
            current_status=current_status,
            task_progress=task_progress,
            current_subtask=last_subtask,
    ...
    def goal_update(
        ...
        for f in self.facts:
            facts_str += f"{str(f)}\n"
        dialogue_history = "\n".join(self.memory)
        goal_update_prompt = self.goal_update_prompt.format(
            robot_name=self.robot_name,
            current_status=current_status,
            task_progress=task_progress,
            current_subtask=last_subtask,
        ...
        for g in updated_goals:
            goal_str += f"{str(g)}\n"
        print()
        print(f"{self.robot_name}'s updated communication purpose:\n{goal_str}")
        print()
    ...
    def act(
        ...
        if "pull" in action:
            if len(misplaced_objects) == 0:
                ...
                prompt = pull_purpose_prompt.format(
                    robot_name=self.robot_name,
                    action=action, 
                    dialogue_history=dialogue_history,
                    action_history=action_history,
                ...
                for i in range(5):
                    ...
                    if trapped_object in misplaced_objects:
                        break
                    else:
...
class ObservationAgent(Agent):
    ...
    def detect_misplaced(self, new_observation: dict):
        ...
        prompt = self.misplaced_detect_prompt.format(
    ...
    def reason_container(self, obj_and_container, known_obj):
        ...
        """
        prompt = self.misplaced_object_container_reason_prompt.format(
    ...
    def act(
        ...
        if len(obj_and_container) != 0:
            # TODO: known_obj 过滤为 placeable 的容器å
...
class RelfectionAgent(Agent):
    ...
    def act(
        ...
        if flag:
            reflection_prompt = self.success_reflection_prompt.format(
                robot_name=self.robot_name,
            ...
            reflection_prompt = self.failed_reflection_prompt.format(
                robot_name=self.robot_name,
                subtask=subtask,
                task_progress=task_progress,
                action_history=action_history,
...
class ProgressAgent(Agent):
    ...
    def after_comm(
        ...
        prompt = self.comm_task_evaluation_prompt.format(
            robot_name=self.robot_name,
            current_status=current_status,
            task_progress=task_progress,
