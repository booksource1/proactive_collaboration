...
class MultiRobotCommunicator:
    ...
    def __init__(
        ...
        self.max_comm = 4  # Max rounds allowed per robot pair (1-2-1-2 is 3 rounds)
        ...
        print(f"Number of robot pairs: {self.num_robot_pairs}")
    ...
    def communicate(self):
        ...
        while communication_queue and iteration < max_iterations:
            ...
            if self.debug:
                print(f"CommunicationAgent Actions for {current_robot}:")
                pprint(messages)
                print("=====================================\n\n")
            ...
            for m in messages:
                ...
                if receivers == ["None"] or message == "None":
                    # Robot chooses to keep silent
                    if self.debug:
                        print(f"{current_robot} chooses to remain silent.")
                        print("=====================================\n\n")
                    continue
    ...
    def update_dialogue_and_queue(
        ...
        if self.comm_counts[pair] > self.max_comm:
            if self.debug:
                print(
                    f"Communication limit reached between {current_robot} and {receiver}"
                )
                print(f"Message Cut: {message}\n")
            return
...
def round_robin_communicate(robot_team:list[Robot], comm_step:int, debug=True):
    ...
    for robot in robot_team:
        ...
        if debug:
            print(f"CommunicationAgent Actions for {robot.name}:")
            pprint(messages)
            print("=====================================\n\n")
        for m in messages:
            receivers = m.get("receiver", ["None"])
            ...
            if receivers == ["None"] or message == "None":
                if debug:
                    print(f"{robot.name} chooses to remain silent.")
                    print("=====================================\n\n")
                continue
