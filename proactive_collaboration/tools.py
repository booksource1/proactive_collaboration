...
# TODO: Load ROOM_LIST and TALK_ALGORITHM from config
def init_config(dataset_index: int, dataset_dir: str):
    """
...
def parse_json_from_response(raw_str, key):
    ...
    try:
        ...
        # [{...}]
        if cleaned_str.startswith("["):  # 处理列表
...
# def parse_json_from_response(raw_str, key):
#     """
#     解析包含在 ```json 和 ``` 之间的 JSON 字符串，并返回指定 key 的值。

#     Args:
#         raw_str (str): 来自 LLM 的原始响应。解析 JSON 字符串。
#         key (str): 要从 JSON 字符串中提取的键。

#     Returns:
#         Any: JSON 字符串中指定键的值。如果解析失败或键不存在，返回 None。

#     Examples:
#         raw_str = 'xxx ```json{"name": "Alice", "age": 25}```xxx'
#         name = parse_json_from_response(raw_str, "name") # "Alice"
#     """
...
#         if not json_match:
#             raise ValueError("未找到 ```json 和 ``` 之间的 JSON 内容。")
...
def process_actions(action: str, mapper: ItemMapper, rooms: list, if_env_name=True):
    ...
    Args:
        action (str): The input action string. eg. "[goto] <room>" 还没有增加针对有direction的处理
