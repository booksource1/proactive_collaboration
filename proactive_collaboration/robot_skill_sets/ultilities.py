...
def is_point_in_quadrilateral(points, point):
    """
    判断点是否在由四个点定义的四边形内
    :param points: 四边形的四个顶点，列表形式 [[x1, y1], [x2, y2], [x3, y3], [x4, y4]]
    :param point: 待检测点 [x, y]
    :return: True 如果点在四边形内，否则 False
    """
    if len(points) != 4:
        raise ValueError("必须提供四个点来定义四边形")

    # 确保点按顺时针或逆时针排列（简单实现，假设用户输入正确）
    polygon = mpath.Path(points)
    return polygon.contains_point(point)


# # 示例
# quad_points = [[0, 0], [4, 0], [3, 3], [1, 3]]  # 定义四边形顶点
# test_point = [2, 2]  # 待检测点

# result = is_point_in_quadrilateral(quad_points, test_point)
# print(f"点 {test_point} {'在' if result else '不在'} 四边形内")
